...
export const authProvider: AuthProvider = {
  ...
  login: async ({ username, password }) => {
    ...
    if(json?.accessToken){
        ...
        localStorage.setItem("user", JSON.stringify({...user, avatar: `${process.env.REACT_APP_IMG}/${user.avatar}`}));
        localStorage.setItem('permissions', user.roles.includes("ADMIN") ? "ADMIN" : "USER");
        return
    }
